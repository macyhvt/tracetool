...
$autocomplete   = $attribs->get('autocomplete', 'off');	// can be one of: "off", "on", "current-password"", new-password", "one-time-code" (see https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/password)
...
$maxLength      = (int) $attribs->get('maxlength');
$minLength      = (int) $attribs->get('minlength');
$name           = $attribs->get('name', 'password');
$pattern        = $attribs->get('pattern', sprintf('.{%d,%d}', ($minLength) ?? FTKPARAM_PASSWORD_MIN_LENGTH, ($maxLength ?? FTKPARAM_PASSWORD_MAX_LENGTH)));	// must be a valid JavaScript regular expression, as used by the RegExp type and documented at https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions
...
<input type="password"
       ...
       <?php if (0) : ?>form="<?php echo $form; ?>"<?php endif; ?>
