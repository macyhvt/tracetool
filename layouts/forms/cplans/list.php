	...
	<div class="col">
		...
		<div class="d-inline-block align-top">
			<div class="input-group">
				...
				<a href="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=cplan&layout=add&return=%s',
						$this->language,
				   ...
				   data-location="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=cplan&layout=add&return=%s',
						$this->language,
				   ...
				   tabindex="<?php echo ++$tabindex; ?>"
				>
					<span title="<?php echo Text::translate('COM_FTK_BUTTON_TITLE_CPLAN_CREATE_TEXT', $this->language); ?>"
						  aria-label="<?php echo Text::translate('COM_FTK_BUTTON_TITLE_CPLAN_CREATE_TEXT', $this->language); ?>"
						  data-toggle="tooltip"
					>
						<span class="btn-text d-none d-md-inline ml-md-1 text-<?php echo ($this->language == 'de') ? 'capitalize' : 'lowercase'; ?>"><?php
							echo Text::translate('COM_FTK_BUTTON_TEXT_CPLAN_ADD_TEXT', $this->language);
						?></span>
					</span>
				</a>
			</div>
			...
			>
				...
				<div class="input-group input-group-md">
					...
					<input type="search"
						   ...
						   placeholder="<?php echo Text::translate('COM_FTK_INPUT_PLACEHOLDER_CPLAN_NUMBER_TEXT', $this->language); ?>"
...
<div class="position-relative" data-items="<?php echo $listCount; ?>">
	...
	<?php array_walk($this->list, function($cplan) { ?>
		<?php // Load data into an entity object. ?>
		<?php $item = Entity::getInstance('cplan', ['language' => $this->language])->bind($cplan); ?>
		...
		>
			<div class="row">
				...
				<div class="col-10 col-xl-auto px-md-0" style="font-size:1rem!important">
					<a href="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=cplan&layout=item&id=%d&return=%s',
							$this->language,
					   ...
					   data-bind="windowOpen"
					   data-location="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=cplan&layout=item&id=%d&return=%s',
						   $this->language,
						   (int) $item->get($item->getPrimaryKeyName()),
						   base64_encode( basename( (new Uri($this->view->get('input')->server->getUrl('REQUEST_URI')))->toString() ) )
					   ))); ?>"
				...
				<div class="col-auto row-actions px-0 px-sm-auto">
					...
					<a href="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=cplan&layout=edit&id=%d&return=%s',
					   ...
					   data-bind="windowOpen"
					   data-location="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=cplan&layout=edit&id=%d&return=%s',
						   $this->language,
						   (int) $item->get($item->getPrimaryKeyName()),
						   base64_encode( basename( (new Uri($this->view->get('input')->server->getUrl('REQUEST_URI')))->toString() ) )
					   ))); ?>"
