...
<?php /* Load view data */
$staticTechParams = ArrayHelper::getColumn($this->staticTechParams, 'name');	// $this->staticTechParams is a list of objects.
																				// For ArrayHelper::arraySearch() to work properly, we require to generate a list of strings first from the 'name' column
$itemTechParams   = array_map(function($param) { return $param->name; }, $this->techParams);
?>

<?php if ($this->user->getFlags() >= User::ROLE_MANAGER) : ?>
	<?php if (is_countable($itemTechParams) && !count($itemTechParams)) : ?>
...
>
	...
	<?php 	//$isStaticField = (!empty($paramValue) && ArrayHelper::arraySearch($paramValue, $this->staticTechParams));	// DiSABLED on 2023-06-20 - replace with next line + newly added var $staticTechParams inline 11 ?>
	<?php 	$isStaticField = (!empty($paramValue) && ArrayHelper::arraySearch($paramValue, $staticTechParams)); ?>
