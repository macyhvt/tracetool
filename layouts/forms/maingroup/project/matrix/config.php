...
>
	...
	<a href="<?php echo UriHelper::osSafe( UriHelper::fixURL(sprintf( 'index.php?hl=%s&view=%s&layout=project.matrix&proid=%d', $this->language, $view->get('name'), $proID ))); // back-link ?>"
	   role="button"
	   class="btn btn-link"
	...
	<fieldset name="colors">
		...
		<?php foreach ($processes as $procID => $process) : ?><?php
				$process = new Registry($process);
				$processName = $process->get('name', '');
				$processAbbr = $process->get('abbreviation', '');

				$styleCellColor      = $matrixConfig->get('colors.cell.' . $processAbbr, '');	// #212529 is the default colour as defined in TWBS stylesheet for the <body/> element
				$styleCellFontFamily = $matrixConfig->get('colors.font.' . $processAbbr, '');
				$styleCellFontWeight = $matrixConfig->get('font.weight.' . $processAbbr, 'normal');

				// Skip blocked or trashed processes. ?>
		...
		<div class="row form-group">
			...
			<div class="col">
				<input type="text"
					   ...
					   value="<?php echo $styleCellFontFamily; ?>"
			...
			<div class="col">
					<label class="btn btn-sm btn-block btn-info btn-checkbox my-0 cell-style-<?php echo html_entity_decode($process->get('abbreviation')); ?><?php echo ($styleCellFontWeight == 'bold' ? ' active' : ''); ?>"
						   role="button"
					...
					>
						<input type="checkbox"
							   ...
							   value="bold"
							   autocomplete="off"
							   <?php echo ($styleCellFontWeight == 'bold' ? ' checked' : ''); ?>
						/>
			...
			<div class="col">
				<input type="text"
					   ...
					   readonly
					   style="<?php echo sprintf('background:%s; color:%s; font-weight:%s', $styleCellColor, $styleCellFontFamily, $styleCellFontWeight); ?>"
				/>
			</div>
