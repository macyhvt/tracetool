...
//$thislastID    = $model->getInstance('processes', ['language' => $lang])->getLastInsertID();
$this->mainGroups = $model->getInstance('maingroup', ['language' => $this->language])->getMainGroupsNew();

$this->mainGroupProjects = $model->getInstance('maingroup', ['language' => $this->language])->getMainGroupProjects();

//print_r($this->mainGroups);
// Init tabindex
$this->tabindex = 0;
?>
...
<form action="<?php echo UriHelper::osSafe( UriHelper::fixURL($formAction->toString())); ?>"
	  ...
	  class="form form-horizontal maingroupForm validate"
...
>
	...
	<?php echo LayoutHelper::render(sprintf('toolbar.item.%s', mb_strtolower($layout)), [
			...
			'hide'       => ['back','submitAndNew','submit'/* ,'cancel' */],
			'user'       => $this->user
		],
	...
	<ul class="nav nav-tabs mt-md-3 mt-lg-4" id="myTab" role="tablist">
		<li class="nav-item" role="presentation">
			...
			echo "Assign subgroups to main group"//echo Text::translate('COM_FTK_LABEL_MASTER_DATA_TEXT', $this->language); ?></a>
		</li>
		<!--<li class="nav-item" role="presentation">
...
/*			echo "Add project";//echo Text::translate('COM_FTK_LABEL_SURVEILLANCE_TEXT', $this->language); */?></a>
		</li>-->
	</ul>
