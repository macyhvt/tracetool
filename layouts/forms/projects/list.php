...
$mainGroupsList1     = array_keys($mainGroups);
	...
	<div class="col">
		...
		>
			...
			>
				...
				<?php foreach ($mainGroupsList as $group) : ?>
				<?php	echo sprintf('<option value="%s"%s>%s</option>', $group, (in_array($group, $mainGroupsFilter) ? ' selected' : ''), $group); ?>
				<?php endforeach; ?>
			</select>
		</form>
		...
		<div class="d-inline-block align-top">
			<div class="input-group">
				...
				<a href<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=project&layout=add', $this->language ))); ?>"
				   role="button"
				   class="btn btn-sm btn-custom align-super left-radius-0 pr-md-3 allow-window-unload"
				   data-bind="windowOpen"
				   data-location="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=project&layout=add&return=%s', $this->language, base64_encode( basename( (new Uri($input->server->getUrl('REQUEST_URI')))->toString() ) ) ))); ?>"
				   style="padding:0.375rem 0.8rem"
				   tabindex="<?php echo ++$tabindex; ?>"
				>
					<span title="<?php echo Text::translate('COM_FTK_BUTTON_TITLE_PROJECT_CREATE_TEXT', $this->language); ?>"
						  aria-label="<?php echo Text::translate('COM_FTK_BUTTON_TITLE_PROJECT_CREATE_TEXT', $this->language); ?>"
						  data-toggle="tooltip"
					>
						<span class="btn-text d-none d-md-inline ml-md-1 text-<?php echo ($this->language == 'de') ? 'capitalize' : 'lowercase'; ?>"><?php
							echo Text::translate('COM_FTK_LABEL_PROJECT_ADD_TEXT', $this->language);
						?></span>
					</span>
				</a>
			</div>
...
<?php if (!$cnt) : ?>
	...
	<?php else : ?>
		<div class='no-results text-center pt-lg-4 pb-lg-5'></div>
	<?php endif; ?>
<?php endif; ?>
...
<div class="position-relative" data-items="<?php echo count($list); ?>">
	...
	<?php array_walk($list, function($project) use(&$view, &$model, &$input, &$user) { ?>
		...
		<li class="list-item<?php //echo (/* $user->getFlags() <= \Nematrack\Access\User::ROLE_MANAGER &&  */ ($item->get('archived') || $item->get('blocked') || $item->get('trashed')) ? ' list-item-hidden d-none' : ''); ?>"
		...
		>
			<div class="row">
				...
				<div class="col-10 col-xl-auto px-md-0" style="font-size:1rem!important">
					<a href="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=project&layout=item&proid=%d', $this->language, $item->get('proID') ))); ?>"
					   class="btn btn-sm btn-link d-inline-block text-left<?php echo ($item->get('trashed') ? ' text-line-through' : ''); ?>"
					   data-bind="windowOpen"
					   data-location="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=project&layout=item&proid=%d&return=%s', $this->language, $item->get('proID'), base64_encode( basename( (new Uri($input->server->getUrl('REQUEST_URI')))->toString() ) ) ))); ?>"
					   data-location-target="_blank"
					   target="_blank"
					   style="min-width:3rem"
					>
						<span title="<?php echo Text::translate('COM_FTK_LINK_TITLE_PROJECT_VIEW_THIS_TEXT', $this->language); ?>"
