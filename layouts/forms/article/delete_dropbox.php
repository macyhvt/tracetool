...
if ($fileId <= 0) {
    ...
    $filePath = Model::getInstance('article')->checkDropBoxFile($fileId);
...
if (!empty($filePath)) {
    ...
    if (file_exists($fileToDelete)) {
        if (unlink($fileToDelete)) {
            ...
            try {
                $fDelete = Model::getInstance('article')->deleteDropBoxFile($fileId);
                header('Content-Type: application/json');
            ...
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'message' => 'Failed to delete file from the server.']);
            exit;
        }
    } else {
        header('Content-Type: application/json');
    ...
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => 'Filepath is missing for deletion']);
    exit;
}
?>