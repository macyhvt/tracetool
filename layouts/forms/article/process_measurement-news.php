...
/*-- To get the actual frequency this code is responsible --*/
$actualMPart   = $model->getInstance('part', ['language' => $this->language])->activeArticleMP($item->get('artID'), $pid); //article measurement //This is needed

if($isTracking && $workinglayout == 'edit' && $workingdir == 'part'):
	$totalNumberOfParts = $model->getInstance('part', ['language' => $this->language])->getTFTM($item->get('artID'), $pid, $input->get('ptid', 0, 'INT'), $partLotID);
	$totalNumberOfParts = array_keys($totalNumberOfParts);

	$mikvals = $model->getInstance('part', ['language' => $this->language])->getHMPNT($item->get('artID'), $pid, $input->get('ptid', 0, 'INT'), $partLotID);

	$ifPartsInLotNew = [];
	foreach($actualMPart as $valos){
		$artLimis= $valos['mpFrequencyScope'];
		$ifPartsInLotNew[$valos['mp']]   = $model->getInstance('part', ['language' => $this->language])->newCheckNeeded($item->get('artID'), $pid, $input->get('ptid', 0, 'INT'), $artLimis, $artLimis= $valos['mp'], $partLotID);
	}
	$mergedArray = $ifPartsInLotNew;
	$mps =$model->getInstance('part', ['language' => $this->language])->getHMPNT($item->get('artID'),$pid, $input->get('ptid', 0, 'INT'),$partLotID);
...
span.frmps {
	...
	if ($isTracking && $workinglayout == 'edit' && $workingdir == 'part'):
		$kioTotal = array(); // Declare $kioTotal outside the loop
		for ($i = 0; $i < count($actualMPart); $i++) {
			$targetMp = $actualMPart[$i]['mp'];
			$matchingPartIDs[$i] = array();
			foreach ($mergedArray[$actualMPart[$i]['mp']] as $itemdd) {
		...
		for($i=0;$i<count($kioTotal);$i++){
			echo "<script>
				$ = jQuery;
				$(document).ready(function (){
					$('#p-{$pid}-{$i}-td-mpFrequency input').each(function (){
						var actualFrequencyValue = {$actualMPart[$i]['mpFrequency']}
						var lotnumber = {$totalNumberOfParts[0]}
						var alreadyTrackedValue = ({$kioTotal[$i]})
						var actualCountNow = lotnumber - alreadyTrackedValue;
                        $(this).attr('realFreaquency','{$actualMPart[$i]['mpFrequency']}');
                        let totalLot = {$totalNumberOfParts[0]};
                        let rfrequency = {$actualMPart[$i]['mpFrequency']}
                        let shownVal = $('#ipt-mpFrequencyScope-{$pid}-{$i}').val();
                     	 
						if(actualFrequencyValue<alreadyTrackedValue){
                            if(actualCountNow == 0){
                                $(this).after('<span class=\"frmp\"> +'+actualFrequencyValue+'</span>'); 
                            }else{
						 		$(this).after('<span class=\"frmp\"> +'+actualCountNow+'</span>');                                
                            }
						}else if(actualFrequencyValue<=alreadyTrackedValue){
                            var newValue = lotnumber - alreadyTrackedValue;
                            if(newValue == 0){
                                $(this).after('<span class=\"frmp\"> +'+actualFrequencyValue+'</span>');
                            }else{
                            	$(this).after('<span class=\"frmp\"> +'+newValue+'</span>');    
                            }
                            ...
                            if(newValue == 0){
                                ...
                                if(shownVal != 1){
                                	var newValue = actualFrequencyValue - alreadyTrackedValue;
                            		$(this).after('<span class=\"frmps\"> -'+newValue+'</span>');
                                }else{
                                    var newValue = actualFrequencyValue - alreadyTrackedValue;
                            		$(this).after('<span class=\"frmp\"> '+newValue+'</span>');
                                }
...
if(isset($mergedArray)) {
	if ($isTracking && $workinglayout == "edit" && $workingdir == "part"):
		$alrtBox = "<div class='measurementRequire'>" . Text::translate('COM_FTK_REQUIRE_MEASUREMENT_PARTSPAGE', $this->language) . "</div>";
		$alrtstyle = '<style>
            .measurementRequire {
                width: 120px;
				height: 110px;
		...
		$actualMPartss   = $model->getInstance('part', ['language' => $this->language])->activeArticleMP($item->get('artID'), $pid);
		...
		for ($i = 0; $i < count($newArray); $i++) {
			$countKeysInCUT1DD004 = $model->getInstance('part', ['language' => $this->language])->countKeys($mergedArray, $newArray[$i]['mp']);//countKeys
			//echo "This is actual-".$newArray[$i]['mpFrequencyScope'];
...
if (true == $isTracking) :
	$thead['th-mike1' ] = '<th class="text-center align-middle" id="th-mike1"  style="padding-bottom:6px; width:90px">' .
		((false == $isTracking) ? '<abbr title="%ABRV11%" data-toggle="tooltip" data-html="true">Go to Parts</abbr>' : 'Go to Parts') .
		'</th>';
endif;
if (true == $isTracking) :
	$thead['th-miketwo' ] = '<th class="text-center align-middle" id="th-miketwo"  style="padding-bottom:6px; width:90px">' .
		((false == $isTracking) ? '<abbr title="%ABRV11%" data-toggle="tooltip" data-html="true">Evidence Parts</abbr>' : 'Evidence Parts') .
		'</th>';
endif;
...
// TODO - translate
$rowDummy = '' .
'<tr style="position: relative" id="p-%PID%-%CNT%-tr">' .
...
<?php // column mp
if (false == $isTracking):
$ptid = $input->get('ptid', 0, 'INT');
$artiID = $input->get('artid', 0, 'INT');

//echo "-----\n\n";
$url = UriHelper::osSafe( UriHelper::fixURL(sprintf('index.php?hl=%s&view=parts&layout=partlist&task=search&ptid=%d&artid=%d&procid=%d',
    $this->language,
    $ptid,
    $artiID,
    $pid
...
if (true == $isTracking):
    $ptid = $input->get('ptid', 0, 'INT');
    $artiID = $input->get('artid', 0, 'INT');
	$matchingPartIDs = array();
//echo "-----\n\n";
    $url = UriHelper::osSafe( UriHelper::fixURL(sprintf('index.php?hl=%s&view=parts&layout=partlist&task=search&ptid=%d&artid=%d&procid=%d&lotid=%d',
        $this->language,
        $ptid,
        $artiID,
        $pid,
...
//			  'placeholder="%PH1%" ' .
        ...
        '<div class="knowBparts" ><a target="_blank" href="'.$url.'&mp=%VALUE%" id="knowallparts" data-mp-id="%VALUE%" data-part-id="'.$ptid.'" data-arti-id="'.$artiID.'" data-proc-id="'.$pid.'">click here</a></div>';
    '</td>';
endif;
?>
...
$fields['mpFrequency'] = '<td id="p-%PID%-%CNT%-td-mpFrequency">' .
	'<input type="' . ($isTracking || $isReadonly ? 'text' : 'number') . '" ' .
		   'class="form-control form-control-sm mpFrequency text-right" ' .
		   'value="%VALUE%" ' .
		   (!$isReadonly
			? 'id="ipt-mpFrequency-%PID%-%CNT%" ' .
...
if (true == $isTracking) :
	$fields['mike1'] = 'cvcx<td id="p-%PID%-%CNT%-td-mike1">' .
		'<input type="' . ($isTracking || $isReadonly ? 'text' : 'number') . '" ' .
		'class="form-control form-control-sm mike1 text-right" ' .
		'value="%VALUE%" ' .
		(!$isReadonly
			? 'id="ipt-mike1-%PID%-%CNT%" ' .
			'name="procMeasurement' . (!$isTracking ? 'Definition' : 'Data') . '[%PID%][%CNT%][%KEY%]" ' .
//			  'placeholder="%PH10%" ' .
			'title="%REQUIRED10%" ' .
			'min="1" ' .
...
if (true == $isTracking) :
	$fields['miketwo'] = '<td id="p-%PID%-%CNT%-td-miketwo">' .
		'<input type="' . ($isTracking || $isReadonly ? 'text' : 'number') . '" ' .
		'class="form-control form-control-sm miketwo text-right" ' .
		'value="%VALUE%" ' .
		(!$isReadonly
			? 'id="ipt-miketwo-%PID%-%CNT%" ' .
			'name="procMeasurement' . (!$isTracking ? 'Definition' : 'Data') . '[%PID%][%CNT%][%KEY%]" ' .
//			  'placeholder="%PH10%" ' .
			'title="%REQUIRED10%" ' .
			'min="1" ' .
...
use (&$debug, &$isTracking, &$isAutoTrackm, &$isReadonly, &$fields, &$measuredData, &$user, &$lang, $definedMPs, $actualMPart, $mergedArray, $workinglayout, $workingdir, $model): string
{
...
//		case ($fieldName == 'mpDatatype') :
	...
	if(isset($mergedArray)) {
	if ($isTracking && $workinglayout == "edit" && $workingdir == "part") :
//echo count($actualMPart);
	for($i=0; $i<count($actualMPart); $i++){
		 $countKeysInCUT1DD004 = $model->getInstance('part', ['language' => $this->language])->countKeys($mergedArray, $actualMPart[$i]['mp']);//countKeys($mergedArray, $actualMPart[$i]['mp']);
		//echo $mkpartsa[$actualMPart[$i]['mp'][$i]['mpcount']];
		//echo "<pre>";print_r($mkpartsa[$actualMPart[$i]['mp']][$i]);
		if($actualMPart[$i]['mpFrequency']>$countKeysInCUT1DD004){//$mkpartsa[$actualMPart[$i]['mp']][$i]['mpcount']){
			if ($fieldName == 'mpInput') :
				//echo $value;
				if (true == $isTracking && ($value === "" || $value === NULL || $value === 0)) :
					...
					echo "<script>
                        ...
                        function checking() {
                            //alert('de');
                            var empty = 0;
                            $('#ipt-mpInput-{$procID}-{$i}').each(function(){
                                let shownVal = $('#ipt-mpFrequencyScope-{$procID}-{$i}').val();
                               if (this.value == '' || this.value == 0) {
                                    empty++;
                                    if(shownVal != 1){
                                    $('#ipt-mpInput-{$procID}-{$i}').addClass('highlight');
                                    }
                                    
                                $('.btn-save').css({pointerEvents:'none',background:'gray',color:'#fff'});
                               }else{
                                   $(this).removeClass('highlight'); 
                                   $('.btn-save').css({pointerEvents:'auto',background:'#ffc107',color:'#212529'});
                               } 
                            })
                             /*var jempty = 0;
                            $('#lst-mpInput-{$procID}-{$i}').each(function(){
                               if (this.value == '' || this.value == 0) {
                                    jempty++;
                                    $('#lst-mpInput-{$procID}-{$i}').addClass('highlight');
                                $('.btn-save').css({pointerEvents:'none',background:'gray',color:'#fff'});
                               }else{
                                   $(this).removeClass('highlight'); 
                                   $('.btn-save').css({pointerEvents:'auto',background:'#ffc107',color:'#212529'});
		...
		}elseif($actualMPart[$i]['mpFrequency']<=$countKeysInCUT1DD004){
			echo "<script>
                        $ = jQuery;
                        $('#ipt-mpInput-{$procID}-{$i}').css({border:'1px solid green',color:'green'});
                       // $('#lst-mpInput-{$procID}-{$i}').css({border:'1px solid green',color:'green'});
                        //console.log($('#ipt-mpInput-{$procID}-0'));
                    </script>";
		}
	}
	endif;
