...
if (false == $isTracking) :
$thead['th-mptargetValueSPC'] = '<th class="text-center align-middle targetvaluehead" id="th-mptargetValueSPC"        style="padding-top:5px;    width:120px">' .
    '<abbr title="%ABRV12%" data-toggle="tooltip" data-html="true">Value</abbr>' .
    '</th>';
endif;
if (false == $isTracking) :
    $thead['th-mpspcvalue'] = '<th class="text-center align-middle cpkvaluehead" id="th-mpspcvalue"        style="padding-top:5px;    width:120px">' .
        '<abbr title="%ABRV12%" data-toggle="tooltip" data-html="true">CPK</abbr>' .
        '</th>';
endif;
if (false == $isTracking) :
    $thead['th-mpconfigBTN'] = '<th class="text-center align-middle" id="th-mpconfigBTN"        style="padding-top:5px;    width:120px">' .
        '<abbr title="%ABRV12%" data-toggle="tooltip" data-html="true"></abbr>' .
        '</th>';
endif;
...
/*$thead['th-targetValue'] = '<th class="text-center align-middle" id="th-targetValue" style="padding-bottom:6px; width:120px">' .
    '<abbr title="Target Value" data-toggle="tooltip" data-html="true">Target Value</abbr>' .
    '</th>';*/
...
//			   (!$isTracking ? '%DISABLED%' : 'readonly') .
    ...
    // Add targetValue column here
    '<td id="p-%PID%-%CNT%-td-mptargetValueSPC">' .
    '<input type="' . ($isReadonly ? 'text' : 'number') . '" ' .
    'class="form-control form-control-sm mptargetValueSPC text-right" ' .
    'id="ipt-mptargetValueSPC-%PID%-%CNT%" ' .
    'name="procMeasurement' . (!$isTracking ? 'Definition' : 'Data') . '[%PID%][%CNT%][mptargetValueSPC]" ' .
    'value="%VALUE%" ' .
    'step="0.001" ' .
//               'placeholder="%PH12%" ' .
    //'title="%REQUIRED12%" ' .
...
$rowDummy = preg_replace(
    [
        '/%ABRV1%/'    ,'/%ABRV2%/'    ,'/%ABRV3%/'    ,'/%ABRV4%/'    ,'/%ABRV5%/'    ,'/%ABRV6%/'    ,'/%ABRV7%/'    ,'/%ABRV8%/'    ,'/%ABRV9%/'    ,'/%ABRV10%/'    ,'/%ABRV11%/','/%ABRV12%/',
        '/%TH1%/'      ,'/%TH2%/'      ,'/%TH3%/'      ,'/%TH4%/'      ,'/%TH5%/'      ,'/%TH6%/'      ,'/%TH7%/'      ,'/%TH8%/'      ,'/%TH9%/'      ,'/%TH10%/'      ,'/%TH11%/',      '/%TH11%/',
        '/%PH1%/'      ,'/%PH2%/'      ,'/%PH3%/'      ,'/%PH4%/'      ,'/%PH5%/'      ,'/%PH6%/'      ,'/%PH7%/'      ,'/%PH8%/'      ,'/%PH9%/'      ,'/%PH10%/'      ,'/%PH11%/'      ,'/%PH12%/',
        '/%OPT1%/'     ,'/%OPT2%/'     ,'/%OPT3%/'     ,'/%OPT4%/'     ,'/%OPT5%/'     ,'/%OPT6%/'     ,'/%OPT10%/'    ,'/%OPT11%/'    ,'/%OPT12%/',
        '/%REQUIRED1%/','/%REQUIRED2%/','/%REQUIRED3%/','/%REQUIRED4%/','/%REQUIRED5%/','/%REQUIRED6%/','/%REQUIRED7%/','/%REQUIRED8%/','/%REQUIRED9%/','/%REQUIRED10%/','/%REQUIRED11%/','/%REQUIRED12%/'
    ],
    [
        // Abbreviations %ABRVx%
        ...
        /* col 12 - mptargetValueSPC  */ Text::translate('COM_FTK_ABBREV_TARGETSPC_SCOPE_TEXT',              $this->language),
        ...
        /* col 12 - mptargetValueSPC  */ Text::translate('COM_FTK_LABEL_TARGET_SPC_TEXT',                         $this->language),
        ...
        /* col 12 - mptargetValueSPC  */ Text::translate('COM_FTK_INPUT_PLACEHOLDER_TARGET_SPC_SCOPE_TEXT',   $this->language),
        ...
        /* col 12 - mptargetValueSPC */ Text::translate('COM_FTK_HINT_TARGET_SPC_REQUIRED_TEXT', $this->language),
    ],
    $rowDummy
);
//echo "<pre>";
//print_r($rowDummy);exit;
?>
...
//			  'placeholder="%PH1%" ' .
    ...
    . (!$isTracking ?
        ...
        '<button type="button"'
        .' id="config-%PID%-%CNT%"'
        .' data-mainid="config-%PID%-%CNT%"'
        .' data-mp="%VALUE%"'
        .' data-artid="'.$articleID.'"'
        .' data-pid="%PID%"'.
        ' class="btn btn-info btnConfigMike2 %VALUE%"' .
        ' data-toggle="tooltip"' .
        //' onclick="showDialog()"' .
        '>' .
        '<span' . ' class="btn-text"' . '>' . Histogram . '</span>' .
        '</button>' .
        '<button type="button"'
        .' id="config-%PID%-%CNT%"'
        .' data-mainid="config-%PID%-%CNT%"'
        ...
        .' data-artid="'.$articleID.'"'
        .' data-pid="%PID%"'.
        ' class="btn btn-info btnConfigMike %VALUE%"' .
        ' data-toggle="tooltip"' .
        //' onclick="showDialog()"' .
        '>' .
        ...
        '<span' .
        ' class="form-control form-control-sm mp text-left inputCPK cpk-value config-%PID%-%CNT%"' .
        ' data-pid="%PID%"'.
        ' data-artid="'.$articleID.'"'.
        ' data-mp="%VALUE%"'.
        ' id="cpk-%PID%-%CNT%"'.
        '></span>'
        : '') .
    '<span class="form-control form-control-sm mp text-left inputCPK up-tol config-%PID%-%CNT%"></span>' .
    '<span class="form-control form-control-sm mp text-left inputCPK low-tol config-%PID%-%CNT%"></span>' .
    '<span class="form-control form-control-sm mp text-left inputCPK nom-val config-%PID%-%CNT%"></span>' .
    '<span class="form-control form-control-sm mp text-left inputCPK mean-value config-%PID%-%CNT%"></span>' .
    '<span class="form-control form-control-sm mp text-left inputCPK std-dev config-%PID%-%CNT%"></span>' .
    '<div class="measure-data-container config-%PID%-%CNT%"></div>' .
...
if (false == $isTracking) :
    $fields['mpDatatype']['boolval'] = '<td id="p-%PID%-%CNT%-td-mpDatatype">' .
        ...
        'class="form-control form-control-sm custom-select custom-select-sm mpDatatype erd text-left" ' .
        'id="lst-mpDatatype-%PID%-%CNT%" ' .
        'size="1" ' .
        'title="%REQUIRED3%" ' .
        '%BINDTO% ' .
...
<?php
if (false == $isTracking) :
$fields['mptargetValueSPC'] = '<td id="p-%PID%-%CNT%-td-mptargetValueSPC">' .
    '<input type="' . ($isReadonly ? 'text' : 'number') . '" ' .
    'class="form-control form-control-sm mptargetValueSPC text-right" ' .
    'value="%VALUE%" ' .
    (!$isReadonly
    ...
    $fields['spcvalue'] = '<td id="p-%PID%-%CNT%-td-spcvalue">' .
        '<span class="myCPK"></span>'.
        '</td>';
endif;
?>
...
//print_r($fields['targetValue']);exit;
//print_r($debug);
//var_dump($fields);
?>
...
foreach ($definedMPs as $pid => $MPs) :
    ...
    foreach ($MPs as $idx => $MP) :
        ...
        $tmp = [];
        $tmp['tr-0' ] = '<tr style="position:relative" id="' . sprintf('p-%d-%d-tr', $pid, $i) . '">';
...
$frmMeasurementDefinitions = preg_replace(
    ...
    [
        ...
        /* col 10 - mpFrequencyScope  */ Text::translate('COM_FTK_HINT_MANDATORY_FIELD_TEXT',		  $this->language),
        /* col 11 - mpToleranceFactor */ Text::translate('COM_FTK_HINT_MEASURED_VALUE_REQUIRED_TEXT', $this->language),
        /* col 12 - mptargetValueSPC */ Text::translate('COM_FTK_HINT_MANDATORY_FIELD_TEXT', $this->language),
    ],
    $frmMeasurementDefinitions
);
?>
...
?>
    <style>
        ...
        .mean-value,.std-dev, .measure-data-container{
            visibility: hidden;
        }
...
?>
    ...
    <div id="dialog" class="dialog-box">
        <style>
            ...
            #dialog .row input[type="date"],
            #dialog .row input[type="number"] {
                padding: 8px;
                border: 1px solid #ccc;
                border-radius: 4px;
                box-sizing: border-box;
                width: auto;             /* Let inputs take natural width */
                margin-right: 10px;    /* Space between inputs if needed */
                flex: 1 1 auto;          /* Grow and shrink, take up available space */
                min-width: 100px;       /* Minimum width so they don't collapse */
            }

            #dialog .row input[type="radio"] {
                margin-right: 5px;
            }

            #dialog .row button[type="submit"],
            #dialog .row button[type="button"] {
            ...
            #dialog .row button[type="submit"]:hover {
                background-color: #3e8e41;
            }

            #dialog .row button[type="button"]:hover {
            ...
            @media (max-width: 1470px) {
                #dialog {
                    width: auto; /* Adjust dialog width to screen size */
                }
                #dialog .row {
                ...
                #dialog .row:nth-child(2) {   /* Select second row */
                    flex-direction: row;        /* Align in a row */
                }

                #dialog .row:nth-child(2) label {  /* select label in second row */
                    width: auto;                 /* let the label width adjust automatically */
                }

                #dialog .row:nth-child(2) input[type="date"] {     /* select input elements with date type in second row */
                    width: auto;
                }
            }
        </style>
        <form id="myForm" method="POST" >
            <input type="hidden" id="dialogMp" name="mp">
            <input type="hidden" id="dialogArtId" name="artid">
            <input type="hidden" id="dialogPid" name="pid">
            <input type="hidden" id="dialogMainID" name="mainid">

            <div class="row">
                <input type="radio" id="dateRangeRadio" name="intervalType" value="dateRange">
                <label for="dateRangeRadio">Interval from - to:</label>
            </div>
            ...
            <div class="row">
                <input type="radio" id="pieceCountRadio" name="intervalType" value="pieceCount">
                <label for="pieceCountRadio">Interval by pieces:</label>
                <input type="number" id="pieceCount" name="pieceCount" min="1">
            </div>

            <div class="row">
                <input type="radio" id="last1000Radio" name="intervalType" value="last1000" checked>
                <label for="last1000Radio">Interval last 1000 pieces:</label>
            </div>
    ...
    <div id="histogramModal" class="modal fade" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 style="text-align: center" class="modal-title">Histogram with Tolerance Window</h5>
    ...
    <script>
        jQuery(document).ready(function($) {
            // Automatically trigger the AJAX call for each .cpk-value element

            $('.cpk-value').each(function() {
                var $this = $(this); // Store the current .cpk-value element

                // Get necessary parameters from data attributes
                var artID = $this.data('artid'); // Get artID from data-artid attribute
                var pid = $this.data('pid'); // Get pid from data-pid attribute
                var mp = $this.data('mp'); // Get mp from data-mp attribute

                // Make the AJAX call
                $.ajax({
                    url: 'index.php?option=com_ftk&view=article&layout=showchart&format=raw',
                    type: 'POST',
                    data: {
                        artID: artID,
                        pid: pid,
                    ...
                    success: function(response) {
                        ...
                        try {
                            ...
                            var stats = typeof response === 'object' ? response : JSON.parse(response);
                            ...
                            // Update the .cpk-value element and other elements with the response data
                            $this.text(stats.cpk);
        ...
        jQuery(document).ready(function($) {
            ...
            function showDialog(button) {
                ...
                // Set form values
                $('#dialogMp').val(mp);
                $('#dialogArtId').val(artId);
                $('#dialogPid').val(pid);
                $('#dialogMainID').val(mainId);
            ...
            // Attach click handler to config buttons
            $('.btnConfigMike').on('click', function() {
                showDialog(this);
            });
            ...
            function toggleInputs() {
                ...
                var pieceCount = $('#pieceCount');

                if ($('#dateRangeRadio').is(':checked')) {
                    dateInputs.show().prop('required', true);
                    pieceCount.hide().prop('required', false);
                } else if ($('#pieceCountRadio').is(':checked')) {
                    dateInputs.hide().prop('required', false);
                    pieceCount.show().prop('required', true);
                } else {
                    dateInputs.hide().prop('required', false);
                    pieceCount.hide().prop('required', false);
                }
            ...
            // Initial toggle and attach change handler
            toggleInputs();
            $('input[name="intervalType"]').on('change', toggleInputs);
            ...
            $('#myForm').on('submit', function(e) {
                ...
                // Validate required fields based on selected interval type
                if (data.intervalType === 'dateRange' && (!data.fromDate || !data.toDate)) {
                    alert('Please select both start and end dates');
                    return;
                } else if (data.intervalType === 'pieceCount' && !data.pieceCount) {
                ...
                // Make AJAX call
                $.ajax({
                    url: 'index.php?option=com_ftk&view=article&layout=cpkvalue&format=raw',
                    type: 'POST',
                    data: data,
                    success: function(response) {
                        var targetClass = data.mainid;
                        try {
                            // Check if response is already an object
                            var stats = typeof response === 'object' ? response : JSON.parse(response);
                            ...
                            $('.' + targetClass + '.cpk-value').text(stats.cpk);
                            $('.' + targetClass + '.mean-value').text(stats.meanValue);
                            $('.' + targetClass + '.std-dev').text(stats.standardDeviation);
                            $('.' + targetClass + '.up-tol').text(stats.upTol);
                            $('.' + targetClass + '.low-tol').text(stats.lowTol);
                            $('.' + targetClass + '.nom-val').text(stats.NomVal);

                            $('.btnConfigMike2[data-mainid="' + targetClass + '"]')
                                .attr('data-measure-data', JSON.stringify(stats.measureData));

                            $('.btnConfigMike2')
                                .attr('data-measure-cpk', JSON.stringify(stats.cpk));

                            // Display measured data
                            var $container = $('.' + targetClass + '.measure-data-container');
                            $container.empty(); // Clear existing data

                            if (stats.measureData && stats.measureData.length > 0) {
                                var html = stats.measureData.map(function(value, index) {
                                    return '<div class="measure-data-item">' +
                                        'Value ' + (index + 1) + ': ' + value +
                                        '</div>';
                                }).join('');
                                $container.html(html);
                            } else {
                                $container.html('<div class="measure-data-item">No data available</div>');
                            }


                            closeDialog();
                            $('#myForm')[0].reset();
                            $('input[name="intervalType"][value="last1000"]').prop('checked', true);
                            toggleInputs();
                        } catch (e) {
                            console.error('Error parsing response:', e);
                            console.error('Raw response:', response);
            ...
            $(document).on('click', '.btnConfigMike2', function() {
                // Get button data
                const $button = $(this);
                const targetClass = $button.data('mainid');

                // Get statistics
                //const cpkValue = parseFloat($('.cpk-value').text());
                var cpkValue = $button.data('measure-cpk');
                //console.log(cpkVl);
                const meanValue = parseFloat($('.mean-value').text());
                const stdDev = parseFloat($('.std-dev').text());
                const measureDataStr = $button.attr('data-measure-data');

                const nominalValue = parseFloat($('.nom-val').text());
                const upperTols = parseFloat($('.up-tol').text());
                const lowerTols = parseFloat($('.low-tol').text());

                //console.log(meanValue);
                //console.log(stdDev);
                const totalCount = measureDataStr.length;

                console.log("Total Count:", totalCount);
                // Validate data exists
                if (!measureDataStr) {
                    alert('No data available for histogram');
                    return;
                }

                // Parse the measured data
                const measureData = JSON.parse(measureDataStr);
                ...
                const bins = Math.floor(Math.sqrt(measureData.length));
                const histogramData = generateHistogramData(measureData, bins);
//console.log(bins);
                // Generate normal distribution curve
                const min = Math.min(...measureData);
                const max = Math.max(...measureData);
                const binWidth = (max - min) / bins;
                const normalCurve = generateNormalDistribution(meanValue, stdDev, min, max);
//console.log(min);
                //console.log(max);
                ...
                const nominalPosition = (nominalValue - min) / binWidth;
                const upperTolPosition = (upperTols - min) / binWidth;
                const lowerTolPosition = (lowerTols - min) / binWidth;

                const totalBins = Math.ceil((max - min) / binWidth);
                const upperTolIndex = Math.min(Math.max(upperTolPosition, 0), totalBins - 1);
                const lowerTolIndex = Math.min(Math.max(lowerTolPosition, -0.4), totalBins - 1);
                const meanPosition = (meanValue - min) / binWidth;
                ...
                window.histogramChart = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        plugins: {
                            title: {
                                display: true,
                                text: `Histogram (Nominal: ${nominalValue.toFixed(3)}, Mean: ${meanValue.toFixed(3)}, StdDev: ${stdDev.toFixed(3)},CPK: ${cpkValue})`
                            },
                            annotation: {
                                annotations: {
                                    line1: {
                                        type: 'line',
                                        xMin: nominalPosition,
                                        xMax: nominalPosition,
                                        borderColor: 'green',
                                        borderWidth: 2,
                                        label: {
                                            content: 'Nominal',
                                            ...
                                            enabled: true,
                                            position: 'top',
                                            xAdjust: 0, // Try these to move horizontal
                                            yAdjust: -30, // Try these to move vertical
                                            color:'green'
                                        }
                                    },
                                    line2: {
                ...
                $('#histogramModal').on('hidden.bs.modal', function (e) {
                    window.location.reload();
                });
            });

// Helper function for histogram data
            function generateHistogramData(data, bins) {
                ...
                const binRanges = new Array(bins);

                // Fill bins
                for (let i = 0; i < bins; i++) {
                    const binStart = min + (i * binWidth);
                    const binEnd = binStart + binWidth;
                    binRanges[i] = `${binStart.toFixed(2)}-${binEnd.toFixed(2)}`;
                }
                ...
                return { counts: histData, labels: binRanges };
            }

// Helper function for normal distribution
            function generateNormalDistribution(mean, stdDev, min, max, points = 100) {
