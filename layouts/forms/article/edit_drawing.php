...
<div class="row form-group ml-sm-0">
	...
	>
		...
		<div class="position-absolute center-block d-block upload-hint" id="drawing-filenumber-cust">
			<button type="button"
					class="position-absolute border-0 fileSelectToggle outline-0 w-100 h-100 d-block<?php echo (!$fileExists) ? ' text-red' : ''; ?>"
					...
					tabindex="<?php echo ++$this->tabindex; ?>"
			><?php    if (!$fileExists) : ?>
					<?php echo Text::translate('COM_FTK_HINT_NO_DRAWING_TEXT', $this->language); ?>
				<?php else : ?>
				<span class="d-block w-100 h-100"
					  title="<?php echo Text::translate('COM_FTK_BUTTON_TITLE_CHANGE_DRAWING_TEXT', $this->language); ?>"
					  aria-label="<?php echo Text::translate('COM_FTK_BUTTON_TITLE_CHANGE_DRAWING_TEXT', $this->language); ?>"
					  data-toggle="tooltip"
				>
...
<div class="row form-group ml-sm-0 mb-0">
	...
	>
		...
		<input type="file"
			   ...
			   <?php // data-rule-required='{"depends":"#ipt-number","strict":"true"}' ?>
			   tabindex="<?php echo ++$this->tabindex; ?>"
			   style="padding-top:.2rem; padding-bottom:.15rem; padding-left:0; border:unset; background:inherit"
		/>
