	...
	<div class="col">
		...
		<div class="d-inline-block align-top">
			<div class="input-group">
				...
				<a href<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=organisation&layout=add', $this->language ))); ?>"
				   role="button"
				   class="btn btn-sm btn-custom align-super left-radius-0 pr-md-3 allow-window-unload"
				   data-bind="windowOpen"
				   data-location="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=organisation&layout=add&return=%s', $this->language, base64_encode( basename( (new Uri($input->server->getUrl('REQUEST_URI')))->toString() ) ) ))); ?>"
				   style="padding:0.375rem 0.8rem"
				   tabindex="<?php echo ++$tabindex; ?>"
				>
					<span title="<?php echo Text::translate('COM_FTK_BUTTON_TITLE_ORGANISATION_CREATE_TEXT', $this->language); ?>"
						  aria-label="<?php echo Text::translate('COM_FTK_BUTTON_TITLE_ORGANISATION_CREATE_TEXT', $this->language); ?>"
						  data-toggle="tooltip"
					>
						<span class="btn-text d-none d-md-inline ml-md-1 text-<?php echo ($this->language == 'de') ? 'capitalize' : 'lowercase'; ?>"><?php
							echo Text::translate('COM_FTK_LABEL_ORGANISATION_ADD_TEXT', $this->language);
						?></span>
					</span>
				</a>
			</div>
...
<div class="position-relative" data-items="<?php echo count($list); ?>">
	...
	<?php array_walk($list, function($organisation) use(&$view, &$model, &$input) { ?>
		<?php // Load data into Registry for less error prone access. ?>
		<?php $item          = new Registry($organisation); ?>

		<?php $itemProcesses = new Registry(json_decode((string) $item->get('processes', ''), null, 512, JSON_THROW_ON_ERROR)); ?>
		...
		>
			<div class="row">
				...
				<div class="col-10 col-xl-auto px-md-0" style="font-size:1rem!important">
					<a href="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=organisation&layout=item&oid=%d', $this->language, $item->get('orgID') ))); ?>"
					   class="btn btn-sm btn-link d-inline-block text-left<?php echo ($item->get('trashed') ? ' text-line-through' : ''); ?>"
					   data-bind="windowOpen"
					   data-location="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=organisation&layout=item&oid=%d&return=%s', $this->language, $item->get('orgID'), base64_encode( basename( (new Uri($input->server->getUrl('REQUEST_URI')))->toString() ) ) ))); ?>"
					   data-location-target="_blank"
					   target="_self"
					>
						<span title="<?php echo Text::translate('COM_FTK_LINK_TITLE_ORGANISATION_VIEW_THIS_TEXT', $this->language); ?>"
							  aria-label="<?php echo Text::translate('COM_FTK_LINK_TITLE_ORGANISATION_VIEW_THIS_TEXT', $this->language); ?>"
							  data-toggle="tooltip"
						><?php
							echo html_entity_decode($item->get('name'));
						?></span>
					</a>
				...
				<div class="col-auto row-actions px-0 px-sm-auto">
					...
					<a href="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=organisation&layout=edit&oid=%d', $this->language, $item->get('orgID') ))); ?>"
					   role="button"
					   class="btn btn-sm btn-link btn-edit"
					   data-bind="windowOpen"
					   data-location="<?php echo UriHelper::osSafe( UriHelper::fixURL( sprintf( 'index.php?hl=%s&view=organisation&layout=edit&oid=%d&return=%s', $this->language, $item->get('orgID'), base64_encode( basename( (new Uri($input->server->getUrl('REQUEST_URI')))->toString() ) ) ))); ?>"
					   data-location-target="_blank"
					   target="_self"
					>
						<i class="fas fa-pencil-alt"></i>
						<span class="sr-only"
							  title="<?php echo Text::translate('COM_FTK_LINK_TITLE_ORGANISATION_EDIT_THIS_TEXT', $this->language); ?>"
