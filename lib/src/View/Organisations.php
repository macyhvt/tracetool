...
class Organisations extends ListView
{
	...
	public function __construct(string $name, array $options = [])
	{
		...
		if ($this->user->getFlags() < \ \Access\User::ROLE_WORKER || ($this->user->isCustomer() || $this->user->isSupplier()))
		{
		...
		switch (true)
		{
			...
			case (!$this->user->isGuest() && !$this->user->isCustomer() && !$this->user->isSupplier() && $this->user->getFlags() >= \ \Access\User::ROLE_MANAGER) :
